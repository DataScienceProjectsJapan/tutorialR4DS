...
```
In the above graph make it clearer as to that each line represents a different value for the drv variable by distinguishing each line with a different color as is done here:
...
Recall that 2-seater cars were somewhat of an anomoly with regard to the relationship between displacement and highway mileage. Write the R code to plot a geom_point graph together with geom_smooth graph, with color=class, excluding all 2seater cars  to generate this graph.
...
```{r D24-hint}
"Recall how filter() was utilized earlier "
```
