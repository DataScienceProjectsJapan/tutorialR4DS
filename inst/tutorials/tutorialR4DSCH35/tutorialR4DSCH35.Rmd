...
#### This tutorial was written by students at Ritsumeikan Asia Pacific University in Japan as part of a collaborative project.
## Section 3.2.4 SD
...
For iris dataset, form a multiplot. Should we use facet_wrap? or facet_grid? Try to run the code and check and decide for yourself which is more informative.
...
Create a new variable called delay which comes from the sum of arr_delay and dep_delay (you could use mutate for this but for this problem use flights$delay). In the next line use summarise to find the mean delay (calling it mean_delay) and remove NAs from the delay using na.rm.
